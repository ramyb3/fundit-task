      ...
      {filteredMatches.map((match) => (
        ...
        <li className="match">
          ...
          <div className="matchData">
            ...
            {/* part C.1 */}
            <input className='button' type="button" value='Approve' onClick={()=> (setStatus([...status, 'approve']), setData([...data, match.id]))}/>
            <input style={{right: '80px'}} className='button' type="button" value='Decline' onClick={()=> (setStatus([...status, 'decline']), setData([...data, match.id]))}/>

            {/* part A */}
            {match.borrower.creditScore>678 ?
              <span className="A"></span> : null
            }
            {match.borrower.creditScore>578 && match.borrower.creditScore<679  ?
              <span className="B"></span> : null
            }
            {match.borrower.creditScore<579 ?
              <span className="C"></span> : null
            }
